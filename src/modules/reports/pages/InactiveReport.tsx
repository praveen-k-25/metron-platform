...
import { getInactiveReport } from "../reports.api";
...
const InactiveReport = () => {
  ...
  const [inactiveData, setInactiveData] = useState<inactiveReportData[]>([]);
  const [inactiveDataCount, setInactiveDataCount] = useState<number>(0);
  const [loading, setLoading] = useState<boolean>(false);
  const [payload, setPayload] = useState<reportForm>();
  //const [search, setSearch] = useState<string>("");

  const inactiveReportForm = useForm({
  ...
  const handleInactiveReport = async (
    ...
    if (
      !payload ||
      Object.keys(inactiveReportForm.formState.errors).length !== 0
    )
      return;
    ...
    try {
      setLoading(true);
      const response = await getInactiveReport(customPayload);
  ...
  return (
    <div className="flex-1 h-full">
      <div className="text-(--report-text) bg-(--report-bg) relative w-full h-full overflow-x-hidden overflow-y-auto flex flex-col gap-5 p-3">
        ...
        >
          <h2 className="text-xl font-semibold px-3 ">Inactive Report</h2>
          <button
            ...
            <table className="text-sm font-normal w-full">
              ...
              <tbody>
                {loading ? (
                  ...
                  <>
                    {inactiveData.map(
                      (data: inactiveReportData, index: number) => (
                        <tr
                        ...
                        >
                          ...
                          <td className="p-2">{data.inactiveStart}</td>
                          <td className="p-2">{data.inactiveEnd}</td>
          ...
          <section className="w-full flex flex-col sm:flex-row gap-3 justify-between items-center p-2 text-sm">
            <div className="text-(--report-text)">
              ...
              <span className="font-semibold">
                {`${1 + rowsperpage.value * (page - 1) > inactiveDataCount ? inactiveDataCount : 1 + rowsperpage.value * (page - 1)} - ${rowsperpage.value * page < inactiveDataCount ? rowsperpage.value * page : inactiveDataCount}`}{" "}
              </span>{" "}
              of <span className="font-semibold">{`${inactiveDataCount}`}</span>
            </div>
            ...
            <div className="flex flex-row gap-2 justify-center items-center">
              <section className="">
                <Select
                  ...
                  onChange={(e: any) => {
                    let temPage = Math.ceil(inactiveDataCount / e.value);
                    if (page > temPage) {
                      handleInactiveReport(temPage, e);
                    } else {
                      handleInactiveReport(page, e);
                    }
                  }}
                  isSearchable={false}
              ...
              <section className="flex justify-center items-center text-(--report-text)">
                ...
                <button
                  disabled={
                    page === Math.ceil(inactiveDataCount / rowsperpage.value)
                  }
                  onClick={() => {
                    //setPage((prev: number) => prev + 1);
                    handleInactiveReport(page + 1);
                  }}
                  className={`border-y border-r border-(--report-border)  rounded-r-lg px-3 p-1  ${page === Math.ceil(inactiveDataCount / rowsperpage.value) ? "text-(--report-hover)" : "hover:bg-(--report-sec-bg)"}`}
                >
                  Next
                </button>
              </section>
